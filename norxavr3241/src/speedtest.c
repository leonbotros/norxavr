...
extern void F(uint32_t S[16], uint8_t rounds); 
...
unsigned long long speedtest_f(uint8_t rounds)
{
  uint32_t S[16] = 
  {
...
void speedtest_f_different_rounds()
{
  int n;
  unsigned long long rounds;
  unsigned long long f_cycles;
  for (n = 0; n < 9; n++)
  {
    rounds = (1UL << n);
    f_cycles = speedtest_f(rounds);
    print("CPU cycles for ");
    printllu(rounds);
    print(" rounds of F: ");
    printllu(f_cycles);
    print(", ");
    printllu(f_cycles/rounds);
    print(" cycles/round");
    print("\n");
  }
  print("\n");
